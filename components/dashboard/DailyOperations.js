import React from "react";
import { styles } from "../../styles";
import { View, Text } from "react-native";
import { ProgressChart } from "react-native-chart-kit";
import { Dimensions } from "react-native";
import { useSelector } from "react-redux";
import { calculatedNumberOfOperations, calculateEstimatedTimeForToday } from "../../helpers";

const { width } = Dimensions.get("window");
const chartConfig = {
  backgroundGradientFrom: "#1E2923",
  backgroundGradientFromOpacity: 0,
  backgroundGradientTo: "#08130D",
  backgroundGradientToOpacity: 0,
  color: (opacity = 1) => `rgba(29, 161, 242,  ${opacity})`,
};


function DailyOperations() {
  
const operationsList = useSelector((state) => state.operations.operationsList);

const totalOperations = calculatedNumberOfOperations(operationsList);
const totalHours = calculateEstimatedTimeForToday(operationsList);
const stringOperation = `${totalOperations} Opérations`;
const stringOperationTime = `${totalHours} heures estimées`;

const data = {
  data: [0.4, 0.8],
};
  return (
    <View style={[styles.defaultContainer, { flex: 1, flexDirection: "row" }]}>
      <ProgressChart
        data={data}
        width={width / 2}
        height={120}
        strokeWidth={8}
        radius={32}
        chartConfig={chartConfig}
        hideLegend={true}
        style={{ alignSelf: "flex-start" }}
      />

      <View style={{ alignItems: "flex-start", paddingTop: 50 }}>
        <Text style={styles.chartText}>{stringOperation}</Text>
        <Text style={styles.chartText}>{stringOperationTime}</Text>
      </View>
    </View>
  );
}

export default DailyOperations;
